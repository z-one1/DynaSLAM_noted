...
vector<ORB_SLAM2::Frame> Geometry::GetRefFrames(const ORB_SLAM2::Frame &currentFrame){
    ...
    for (int i(0); i < mDB.mNumElem; i++){
        ...
        //* 计算两个欧拉教的L2范数，squrt(dYaw^2 + dPitch^2 + dRoll^2)
    ...
    //* 0.7权重的距离差值 + 0.3权重的角度差值 = 算出的两帧之间的差异程度
    ...
    //* MAX_REF_FRAMES：5 
    //* vDist.rows >= 5
    mnRefFrames = std::min(MAX_REF_FRAMES,vDist.rows); //* 就是5
...
vector<Geometry::DynKeyPoint> Geometry::ExtractDynPoints(const vector<ORB_SLAM2::Frame> &vRefFrames,
    ...
    for (int i(0); i < mnRefFrames; i++)
    {
        ...
        //* 保存这个关键帧中的所有有效关键点的信息
        cv::Mat matRefFrame(refFrame.N,3,CV_32F);           //* (u,v,1)
        cv::Mat matDepthRefFrame(refFrame.N,1,CV_32F);      //* d
        cv::Mat matInvDepthRefFrame(refFrame.N,1,CV_32F);   //* 1./d
        cv::Mat vLabels(refFrame.N,1,CV_32F);               //* i
        int k(0);
        //* 遍历这个关键帧中的所有关键点
