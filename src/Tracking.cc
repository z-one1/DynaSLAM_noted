...
cv::Mat Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const cv::Mat &maskLeft, const cv::Mat &maskRight,const double &timestamp)
{
    ...
    /*
        ...
        'mask'：可选掩码
    */
    _mImGray.copyTo(mImGray,imMaskLeft); //* 将mask中为0的部分（先验动态部分）变为0,保留其他部分，结果储存到'mImGray'中
    ...
    else
    {
        ...
        if(bOK)
        {
            ...
            if(!mLastFrame.mTcw.empty())
            {
                ...
                mVelocity = mCurrentFrame.mTcw*LastTwc; //* Twc0 * Tc0c1 = Twc1
            }
            else
                mVelocity = cv::Mat();
        ...
        bool bOK;
        {
            ...
            else
            {
                ...
                else 
                {
                    ...
                    //* 如果重定位失败，匀速运动模型跟踪成功，使用匀速运动模型的跟踪结果
                    if(bOKMM && !bOKReloc)
                    {
                        mCurrentFrame.SetPose(TcwMM);
                        mCurrentFrame.mvpMapPoints = vpMPsMM;
...
void Tracking::UpdateLastFrame()
{
    ...
    vDepthIdx.reserve(mLastFrame.N); //* 分配内存：上一帧特征点的个数
    for(int i=0; i<mLastFrame.N;i++)
    {
    ...
    for(size_t j=0; j<vDepthIdx.size();j++)
    {
        ...
        //* 超过深度阈值的点不要；如果点不够100个，就存入最近的100个
...
bool Tracking::Relocalization(int update)
{
    ...
    for(int i=0; i<nKFs; i++)
    {
        ...
        {
            //* 利用词袋对当前关键帧'mCurrentFrame'和候选关键帧'pKF'的特征点进行匹配，返回成功匹配的数量
    ...
    while(nCandidates>0 && !bMatch)
    {
        for(int i=0; i<nKFs; i++)
        {
            ...
            if(!Tcw.empty())
            {
                ...
                if(nGood<50)
                {
                    //* 通过投影的方式，将候选关键帧中未匹配的地图点投影到当前帧，生成新的匹配
                    ...
                    if(nadditional+nGood>=50)
                    {
                        ...
                        if(nGood>30 && nGood<50)
                        {
                            ...
                            //* 因为已经优化两次了，理论上位姿应该比较准确了，就在更小的范围内寻找候选关键帧，通过投影的形式生成新的匹配
